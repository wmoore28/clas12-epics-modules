...
void S7nodaveMultiBinarySupport::initMaskInternal(dbCommon *record, epicsUInt32& mask, epicsInt16& nobt, epicsUInt16 shft, s7nodavePlcDataSize plcDataSize) {
    int dataSizeInBits = S7nodavePlcAddress::dataSizeInBits(plcDataSize);
    if (nobt < 0 || nobt >= dataSizeInBits) {
        nobt = 0;
        mask = 0;
    }
    if (shft > 0) {
